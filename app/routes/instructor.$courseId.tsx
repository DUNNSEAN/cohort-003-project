...
const courseEditorActionSchema = v.variant("intent", [
  ...
  v.object({ intent: v.literal("rename-module"), moduleId: v.pipe(v.unknown(), v.transform(Number), v.integer()), title: v.pipe(v.string(), v.trim(), v.minLength(1, "Module title cannot be empty.")) }),
  v.object({ intent: v.literal("delete-module"), moduleId: v.pipe(v.unknown(), v.transform(Number), v.integer()) }),
  v.object({ intent: v.literal("add-lesson"), moduleId: v.pipe(v.unknown(), v.transform(Number), v.integer()), title: v.pipe(v.string(), v.trim(), v.minLength(1, "Lesson title cannot be empty.")) }),
  v.object({ intent: v.literal("rename-lesson"), lessonId: v.pipe(v.unknown(), v.transform(Number), v.integer()), title: v.pipe(v.string(), v.trim(), v.minLength(1, "Lesson title cannot be empty.")) }),
  v.object({ intent: v.literal("reorder-modules"), moduleIds: v.pipe(v.string(), v.minLength(1, "Missing module IDs.")) }),
  v.object({ intent: v.literal("reorder-lessons"), moduleId: v.pipe(v.unknown(), v.transform(Number), v.integer()), lessonIds: v.pipe(v.string(), v.minLength(1, "Missing lesson IDs.")) }),
  v.object({ intent: v.literal("move-lesson"), lessonId: v.pipe(v.unknown(), v.transform(Number), v.integer()), targetModuleId: v.pipe(v.unknown(), v.transform(Number), v.integer()), targetPosition: v.pipe(v.unknown(), v.transform(Number), v.integer()) }),
  v.object({ intent: v.literal("delete-lesson"), lessonId: v.pipe(v.unknown(), v.transform(Number), v.integer()) }),
